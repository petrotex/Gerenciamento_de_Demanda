...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    has_senha_gov = serializers.SerializerMethodField(read_only=True)  # somente leitura

    class Meta:
        model = CustomUser
        fields = ['username', 'email', 'cpf', 'password', 'password2', 'senha_gov', 'has_senha_gov']
        extra_kwargs = {
            'email': {'required': True},
            'cpf': {'required': True},
    ...
    def get_has_senha_gov(self, obj):
        return bool(obj.senha_gov)
    ...
    def create(self, validated_data):
        ...
        if senha_gov:
            senha_gov = make_password(senha_gov)
...
class CustomUserSerializer(serializers.ModelSerializer):
    has_senha_gov = serializers.SerializerMethodField(read_only=True)  # só leitura

    class Meta:
        model = CustomUser
        fields = ['id', 'username', 'email', 'cpf', 'has_senha_gov', 'password']
        extra_kwargs = {
            'password': {'write_only': True},
            'senha_gov': {'write_only': True, 'required': False}  # esconde o campo real
        }
